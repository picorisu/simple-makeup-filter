...
## 概要（132文字以内）
Google Meet のカメラ映像に美肌・メイク・シェーディングを。全てローカル処理で、映像が外部に送信されることは一切ありません。

## 詳細説明

Google Meet の自分の映像に、自然な美肌補正とフルメイクをかける拡張機能です。
...
## プライバシーポリシー URL
（未定。ストア公開を決めたときにホスティング先を決定する。
候補: 開発者用 Google アカウントの Google Sites / picorisu.github.io（名義方針が固まったら）。
文面は docs/privacy-policy.md に準備済み。本文中の GitHub 連絡先リンクも置き場所決定時に差し替えること）
